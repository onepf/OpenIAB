...
//    IwTrace(OPENIAB, ("STORE_SKU=%s", [StringUtils CStringCopy:storeSku]));
...
{
    // TODO: combine inapps and subs
    //s3eIOSAppStoreBillingRequestProductInformation(inAppProducts, numInAppProducts);
}
...
void ConvertPurchase(SKPaymentTransaction* transaction, s3eOpenIabPurchase* p)
{
// TODO:
...
- (void) productsLoaded:(NSArray*)products invalidIdentifiers:(NSArray*)invalidIdentifiers
{
    ...
    //IwTrace(OPENIAB, ("Products ARE Loaded. N_VALID=%d N_INVALID=%d", [_identifierProductMapping count], [invalidIdentifiers count]));
...
- (void) productsNotLoaded
{
    ...
    //IwTrace(OPENIAB, ("Products NOT Loaded. COUNT=%s", [StringUtils CStringCopy:error]));
