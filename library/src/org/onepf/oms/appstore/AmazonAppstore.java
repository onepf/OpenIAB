...
public class AmazonAppstore extends DefaultAppstore {
    ...
    public boolean isPackageInstaller(String packageName) {
        if (OpenIabHelper.isDebugEnabled()) Log.d(TAG, "isPackageInstaller() packageName: " + packageName);
        if (sandboxMode != null) {
            return !sandboxMode;
        }
        sandboxMode = !hasAmazonClasses();
        if (OpenIabHelper.isDebugEnabled()) Log.d(TAG, "isPackageInstaller() sandBox: " + sandboxMode);
        return !sandboxMode;
    }
    ...
    public static boolean hasAmazonClasses() {
        ...
        synchronized (AmazonAppstore.class) {
            try {
                ...
                if (OpenIabHelper.isDebugEnabled()) Log.d(TAG, "hasAmazonClasses() cannot load class com.amazon.android.Kiwi ");
                result = false;
            }
        }
        return result;
